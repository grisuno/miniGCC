...
> Generated offline by **readmenator**. 45 files, 718 symbols, 36 imports. Supports C, C++, Python, Go, Rust, JS/TS, Java, C#, Shell, PHP, Dart, GDScript, Nim, ASM, Ruby, Swift, Kotlin, Scala, Lua, Elixir.
> No LLMs. No tokens. Pure static analysis. See more [here](https://github.com/grisuno/ReadMenator)

**Start here:** Statistics Dashboard for scope, God Nodes for blast radius, Architecture Reference for per-file API. Agents: prefer `readmenator-agent/INDEX.md` + `SYMBOLS.md`.

**Total Files Parsed:** 45 | **Total Symbols Extracted:** 718 | **Total Imports:** 36
 | **Resolved Imports:** 4

<!-- ranking_model: v1.0 | weights: {ppr:0.45,auth:0.2,test:0.15,doc:0.1,fresh:0.1} | alpha:0.85 | commit:4e64840 | date:2026-07-18 -->
...
- `minigcc.c` (c, 127 symbols)
- `minigccg2.s` (s, 163 symbols)
- `minigccg3.s` (s, 163 symbols)
- `minigccg4.s` (s, 163 symbols)
...
|------|-------|-------------|----------|
| `minigccg2.s` | 16.3 | | 0.0000 |
| `minigccg3.s` | 16.3 | | 0.0000 |
| `minigccg4.s` | 16.3 | | 0.0000 |
| `minigcc.c` | 12.7 | | 0.0000 |
...
|------|-----------|------------|----------|---------|-------------|
| `t_inner_h.h` | 0.018 | 0.250 | 0.157 | 3 | 1 |
| `my_library.h` | 0.012 | 0.500 | 0.305 | 2 | 2 |
| `t_inline_h.h` | 0.012 | 0.250 | 0.155 | 2 | 1 |
| `test_for.c` | 0.006 | 0.250 | 0.152 | 1 | 1 |
| `t_include.c` | 0.012 | 0.750 | 0.455 | 2 | 3 |
| `t_outer_h.h` | 0.012 | 0.750 | 0.455 | 2 | 3 |
| `test_include.c` | 0.018 | 0.750 | 0.457 | 3 | 3 |
| `test.sh` | 0.000 | 0.000 | 0.000 | 0 | 0 |
| `t_args.c` | 0.012 | 0.250 | 0.155 | 2 | 1 |
| `t_arith.c` | 0.012 | 0.250 | 0.155 | 2 | 1 |
| `minigcc.c` | 0.779 | 1.000 | 0.912 | 127 | 4 |
| `t_inline.c` | 0.031 | 0.750 | 0.462 | 5 | 3 |
| `minigccg2.s` | 1.000 | 0.000 | 0.400 | 163 | 0 |
| `minigccg3.s` | 1.000 | 0.000 | 0.400 | 163 | 0 |
| `minigccg4.s` | 1.000 | 0.000 | 0.400 | 163 | 0 |
...
|---------|----------|-------------|----------|---------|
| `RM001` | info | Large number of functions in c: 180 total | c | 180 |
| `RM002` | info | Large number of functions in s: 489 total | s | 489 |
...
- `minigccg2.s` (163 symbols, no doc)
- `minigccg3.s` (163 symbols, no doc)
- `minigccg4.s` (163 symbols, no doc)
...
- `arg_reg` (line 1633) `static const char *arg_reg(int i)` - *Argument/parameter register names by ABI index. Written as a function instead of a local array literal because the compiler does not allocate brace-initialized local arrays correctly (they overlap adjacent locals).*
- `libc_global_name` (line 1643) `static const char *libc_global_name(int i)` - *Argument/parameter register names by ABI index. Written as a function instead of a local array literal because the compiler does not allocate brace-initialized local arrays correctly (they overlap adjacent locals). static const char *arg_reg(int i) { if (i == 0) return "%rdi"; if (i == 1) return "%rsi"; if (i == 2) return "%rdx"; if (i == 3) return "%rcx"; if (i == 4) return "%r8"; return "%r9"; } /* Predefined libc global symbol names, indexed; returns NULL past the end.*
- `unary` (line 1655) `static void unary(void)`
- `lvalue_address` (line 1927) `static void lvalue_address(void)`
- `handle_postfix` (line 1979) `static void handle_postfix(int is_lvalue)`
- `unary_expr` (line 2113) `static void unary_expr(void)`
- `multiplicative_expr` (line 2128) `static void multiplicative_expr(void)`
- `additive_expr` (line 2192) `static void additive_expr(void)`
- `shift_expr` (line 2247) `static void shift_expr(void)`
- `relational_expr` (line 2266) `static void relational_expr(void)`
- `equality_expr` (line 2319) `static void equality_expr(void)`
- `bitwise_and_expr` (line 2368) `static void bitwise_and_expr(void)`
- `bitwise_xor_expr` (line 2380) `static void bitwise_xor_expr(void)`
- `bitwise_or_expr` (line 2392) `static void bitwise_or_expr(void)`
- `logical_and_expr` (line 2404) `static void logical_and_expr(void)`
- `logical_or_expr` (line 2424) `static void logical_or_expr(void)`
- `conditional_expr` (line 2444) `static void conditional_expr(void)`
- `emit_compound_op` (line 2462) `static void emit_compound_op(int op, int asize)`
- `assignment_expr` (line 2503) `static void assignment_expr(void)`
- `parse_asm_block` (line 2699) `static void parse_asm_block(void)`
- `statement` (line 2714) `static void statement(void)`
- `parse_function` (line 3381) `static void parse_function(const char *name, int ret_type)`
- `parse_enum` (line 3528) `static void parse_enum(void)`
- `skip_struct` (line 3577) `static void skip_struct(void)`
- `skip_typedef` (line 3641) `static void skip_typedef(void)`
- `data_directive` (line 3696) `static const char *data_directive(int size)` - *s->const_value = 8;  /* just a marker /* If a struct was just parsed, store its size if (struct_total_size > 0) s->const_value = struct_total_size; { int h = hash_name(last_name); s->next_hash = hash_table[h]; hash_table[h] = symbol_count - 1; } } match(';'); } /* Storage directive for a datum of `size` bytes.*
- `emit_global_bss` (line 3704) `static void emit_global_bss(const char *name, int is_static, int size)` - *} } match(';'); } /* Storage directive for a datum of `size` bytes. static const char *data_directive(int size) { if (size == 1) return "    .byte %d"; if (size == 2) return "    .word %d"; if (size == 4) return "    .long %d"; return "    .quad %d"; } /* Reserve zero-initialized storage for a global.*
- `emit_global_data_head` (line 3711) `static void emit_global_data_head(const char *name, int is_static)`
- `parse_const_int` (line 3721) `static int parse_const_int(long long *out)` - *Parse an integer constant usable as a static initializer: an optionally signed numeric or character literal, or a macro standing for one. * Returns 1 when a constant was consumed.*
- `intern_string` (line 3744) `static int intern_string(const char *text)` - *} if (tok == T_ID) { int mi = find_macro(token); if (mi >= 0) { long long v = macros[mi].value; next_token(); out = neg ? -v : v; return 1; } } return 0; } /* Record a string literal in the pool and return its label index.*
- `emit_global_initializer` (line 3761) `static int emit_global_initializer(const char *name, int is_static, int *size,
                  ...` - *Emit the definition of a global that carries an initializer. On entry the current token is the one after '='. `size` is the declared byte size and is updated in place when the initializer determines the length of an unsized array. Returns 1 when the initializer was materialized, 0 when the form is unsupported, in which case nothing was emitted and the caller falls back to * zero-initialized storage.*
- `parse_program` (line 3825) `static void parse_program(void)`
- `emit_float_consts` (line 3980) `static void emit_float_consts(void)`
- `emit_string_pool` (line 3990) `static void emit_string_pool(void)`
- `main` (line 4000) `int main(int argc, char **argv)`
...
- `strcpy` (line 2508) `strcpy(saved_token, token);`
- `fputs` (line 2706) `fputs(token, output);`
- `memcpy` (line 2766) `memcpy(varname, token, nlen);`
...
**Functions:**
- `main` (line 1) `int main(void)`
- `volatile` (line 3) `__asm__ volatile("mov %0, %%eax" : "=r"(x));`
...
- `parse_asm_block` (line 25107)
- `statement` (line 25254)
- `restart_typedef` (line 28773)
- `restart_int` (line 29805)
- `parse_function` (line 30964)
- `parse_enum` (line 32429)
- `skip_struct` (line 32841)
- `skip_typedef` (line 33427)
- `data_directive` (line 33917)
- `emit_global_bss` (line 33967)
- `emit_global_data_head` (line 34058)
- `parse_const_int` (line 34113)
- `intern_string` (line 34276)
- `emit_global_initializer` (line 34369)
- `parse_program` (line 35086)
- `emit_float_consts` (line 36891)
- `emit_string_pool` (line 36990)
- `main` (line 37092)
- `_start` (line 40790)
